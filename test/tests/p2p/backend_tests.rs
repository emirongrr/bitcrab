...
async fn test_mock_node_strict_drop() {
    ...
    let server_task = tokio::spawn(async move {
        ...
        let garbage = [0xFF; 50];
        socket.write_all(&garbage).await
...
match read_result {
    ...
    Ok(Ok(n)) => {
        ...
        eprintln!("[mock] received {} bytes from bitcrab (expected: version message)", n);
    ...
    match connect_result {
        Err(_elapsed) => {
            panic!("[bitcrab] FAIL: connect_addr timed out — handshake hung instead of failing fast");
        }
        Ok(Ok((_peer, _rx))) => {
