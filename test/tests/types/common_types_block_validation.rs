...
use bitcrab_common::types::hash::{hash256, BlockHash, Hash256};

#[test]
fn test_block_header_fields() {
    let header = BlockHeader {
        version: 1,
        prev_hash: BlockHash::from_bytes(hash256(&[0u8; 32])),
        merkle_root: Hash256::from_bytes(hash256(&[1u8; 32])),
        time: 1231469665,
        bits: 0x1d00ffff,
        nonce: 0,
    };
    ...
    assert_ne!(header.prev_hash.as_bytes(), header.merkle_root.as_bytes());
}
...
fn test_genesis_block_hash() {
    ...
    assert!(!genesis_hash.iter().all(|&b| b == 0));
}
