...
async fn test_sequential_consensus_sync() {
    ...
    for (i, b) in [&b0, &b1, &b2].iter().enumerate() {
        store
            .store_header(b.header.clone(), BlockHeight(i as u32), i == 2)
            .expect("Store header failed");
    }
    ...
    for (i, b) in [(&b1, 1), (&b2, 2), (&b0, 0)] {
        ...
        store
            .store_block(b.header.clone(), BlockHeight(i), raw)
            .await
            .expect("Store block failed");
    }

    // 4. Notify arrival in out-of-order sequence
    handles
        .block_notifier
        .send((b1.header.block_hash(), BlockHeight(1)))
        .await
        .unwrap();
    handles
        .block_notifier
        .send((b2.header.block_hash(), BlockHeight(2)))
        .await
        .unwrap();
    ...
    handles
        .block_notifier
        .send((b0.header.block_hash(), BlockHeight(0)))
        .await
        .unwrap();
