use bitcrab_net::p2p::codec::{decode_header, encode_header};
...
impl MockPeer {
    ...
    pub async fn bind_and_accept(listener: &TcpListener, magic: Magic) -> Self {
        let (stream, _) = listener
            .accept()
            .await
            .expect("MockPeer failed to accept connection");
        Self { stream, magic }
    }
    ...
    pub async fn send_msg<M: BitcoinMessage>(&mut self, msg: &M) {
        ...
        self.stream
            .write_all(&full_msg)
            .await
            .expect("MockPeer failed to send valid message");
    }
    ...
    pub async fn send_raw(&mut self, garbage: &[u8]) {
        self.stream
            .write_all(garbage)
            .await
            .expect("MockPeer failed to send raw bytes");
    }
