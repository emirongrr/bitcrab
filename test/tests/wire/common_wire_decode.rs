use bitcrab_common::wire::{
    decode::*,
    encode::{BitcoinEncode, Encoder, VarInt, VarStr},
    DecodeError, U16BE,
};
...
impl BitcoinEncode for Simple {
    fn encode(&self, enc: Encoder) -> Encoder {
        enc.encode_field(&self.a).encode_field(&self.b)
    }
}
...
#[test]
fn varint_1_byte() {
    let bytes = Encoder::new().encode_field(&VarInt(42)).finish();
    assert_eq!(bytes, [42]);
}
...
fn varint_3_byte() {
    ...
    let bytes = Encoder::new().encode_field(&VarInt(300)).finish();
...
fn optional_field_absent() {
    ...
    assert_eq!(v, 42);
    let (opt, _dec): (Option<u32>, _) = dec.decode_optional_field().unwrap();
...
fn noncanonical_varint_is_rejected() {
    ...
    let err = match Decoder::new(&bytes).read_varint("v") {
        Ok(_) => panic!("non-canonical varint should fail"),
        Err(err) => err,
    };
    assert!(matches!(
        err,
        DecodeError::NonCanonicalVarint { value: 0xFC, .. }
    ));
}

#[test]
fn bitcoin_core_compactsize_noncanonical_cases() {
    // Mirrors Bitcoin Core `serialize_tests.cpp::noncanonical`.
    let invalid_cases = [
        &[0xfd, 0x00, 0x00][..],             // zero in three bytes
        &[0xfd, 0xfc, 0x00][..],             // 0xfc in three bytes
        &[0xfe, 0x00, 0x00, 0x00, 0x00][..], // zero in five bytes
        &[0xfe, 0xff, 0xff, 0x00, 0x00][..], // 0xffff in five bytes
    ...
    for bytes in invalid_cases {
        assert!(matches!(
            Decoder::new(bytes).read_varint("compact_size"),
            Err(DecodeError::NonCanonicalVarint { .. })
        ));
    }

    let (value, dec) = Decoder::new(&[0xfd, 0xfd, 0x00])
        .read_varint("compact_size")
        .unwrap();
    dec.finish("compact_size").unwrap();
    assert_eq!(value, 0xfd);
}

#[test]
fn bitcoin_core_primitive_serialized_sizes() {
    // Mirrors Bitcoin Core `serialize_tests.cpp::sizes` for primitive wire sizes.
    assert_eq!(Encoder::new().encode_field(&0u8).finish().len(), 1);
    assert_eq!(Encoder::new().encode_field(&0u32).finish().len(), 4);
    assert_eq!(Encoder::new().encode_field(&0u64).finish().len(), 8);
    assert_eq!(Encoder::new().encode_field(&0i32).finish().len(), 4);
    assert_eq!(Encoder::new().encode_field(&0i64).finish().len(), 8);
    assert_eq!(Encoder::new().encode_field(&false).finish().len(), 1);
    assert_eq!(Encoder::new().encode_field(&[0u8; 2]).finish().len(), 2);
}
