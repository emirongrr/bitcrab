...
use std::sync::Arc;
use bitcrab_common::types::{hash::BlockHash, block::BlockHeight};
use bitcrab_net::p2p::actor::{Actor, ActorError, Context};
use bitcrab_storage::Store;
use crate::consensus::validator::TransactionValidator;
use crate::consensus::coins_view::{StoreCoinsView, CoinsViewCache};
...
impl ChainManager {
    ...
    async fn drain_queue(&mut self) -> Result<(), ActorError> {
        loop {
            ...
            let current_hash = self.store.get_best_block()
            ...
            let Some(raw_block_bytes) = self.store.get_block(&hash).map_err(|e| ActorError::Internal(e.to_string()))? else {
                warn!("[chain-manager] block data missing for hash {}. re-downloading...", hash);
                break;
            };

            // 4. Decode block
            use bitcrab_common::wire::decode::{BitcoinDecode, Decoder};
            let (block, _) = bitcrab_common::types::block::Block::decode(Decoder::new(&raw_block_bytes))
                .map_err(|e| ActorError::Internal(format!("failed to decode block: {}", e)))?;

            // 5. Process block (Consensus Validation + UTXO update)
            info!("[chain-manager] attempting to connect block {} at height {}", hash, next_height);
            ...
            match TransactionValidator::connect_block(&block, next_height, &mut cache_view) {
                Ok((_fees, undo)) => {
                    // Store undo data
                    self.store.store_undo(hash, undo).await
                        .map_err(|e| ActorError::Internal(e.to_string()))?;
        ...
        async move {
            match msg {
                ChainMessage::BlockDownloaded(hash, height) => {
                    debug!("[chain-manager] block {} downloaded at height {}", hash, height);
...
mod tests {
    ...
    async fn test_sequential_drain() {
        ...
        // 1. Index headers
        store.store_header(b0.header.clone(), BlockHeight(0), false).unwrap();
        store.store_header(b1.header.clone(), BlockHeight(1), true).unwrap();
        ...
        store.store_block(b1.header.clone(), BlockHeight(1), raw1).await.unwrap();
        manager.waiting_blocks.insert(BlockHeight(1), b1.header.block_hash());
        ...
        store.store_block(b0.header.clone(), BlockHeight(0), raw0).await.unwrap();
        manager.waiting_blocks.insert(BlockHeight(0), b0.header.block_hash());
        
        manager.drain_queue().await.unwrap();
        
        // Hashes should match
        assert_eq!(store.get_best_block().unwrap(), Some(b1.header.block_hash()));
    }
}
